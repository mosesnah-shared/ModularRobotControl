...
% The data directory for MuJoCo
data_dir = '../data/MuJoCo/';

% Figure directory for saving:
fig_dir = '../images/2DOF_singularity/';

%% (Main) For 2DOF robot, R to L

close all
data = load( data_dir + "2DOF_sing_RLR.mat" );
...
%     title('Two-Dimensional Projection of a Torus');
    ...
    if tmp_num == 1
        % Vector or pdf cannot be used due to large size
        exportgraphics( f, [ fig_dir, '2DOF_R2L_', num2str( cnt ), '.jpeg' ] );
    else
        exportgraphics( f, [ fig_dir, '2DOF_L2R_', num2str( cnt ), '.jpeg' ] );
    end
    
    cnt = cnt + 1;
end

%% (BACKUP) For 2DOF movement using RRR
